<template>
        ...
        <div class="flex flex-col justify-center items-center pt-10">
            ...
            <div class="mt-10">
                <label for="default-toggle" class="inline-flex relative items-center cursor-pointer">
                    <span class="mr-3 text-lg font-bold" :class="[isLoadFromLocal ? 'text-green-500' : 'text-gray-500']"
                        >Use Local
                    </span>
                    <input
                        type="checkbox"
                ...
                >
                    <div class="float-left flex justify-start items-center gap-10">
                        ...
                        <input
                            ref="inputUpdateRefs"
                            :class="{
                                'disabled:line-through disabled:decoration-red-500': todo.isFinish,
                            }"
                    ...
                    <div class="float-right mt-2 flex gap-3">
                        <button class="text-lg hover:text-red-500" @click="toggleUpdate(todo.id, index)">
                            <font-awesome-icon icon="fa-solid fa-pen" />
                        </button>
                        <button class="text-lg hover:text-red-500" @click="handler.deleteTask(todo.id)">
    ...
    <template v-else>
        <div class="h-full flex flex-col justify-center items-center">
            <h1 class="mb-10 font-bold text-2xl tracking-wide">You need login to access data from thin backend</h1>
        </div>
    </template>
</template>
...
const todos = computed(() => (loadDataFrom.value === EStore.LOCAL ? todoLocal.todos : todoThinBackend.todos.value))
